...
export default async function OrganizationsPage({ searchParams }: { searchParams: Promise<{ ok?: string; error?: string }> }) {
  ...
  return (
    <AppShell user={user}>
      <PageHeader eyebrow="Estructura" title="Organizaciones" description="Separa campañas y accesos por empresa para mantener el alcance de cada responsable." />
      ...
      <div className="content-grid">
        <section className="card">
          ...
          {organizations.length ? <div className="table-wrap"><table><thead><tr><th>Organización</th><th>NIT</th><th>Sector / ciudad</th><th>Campañas</th><th>Registro</th></tr></thead><tbody>{organizations.map((organization) => <tr key={organization.id}><td><strong>{organization.name}</strong></td><td>{organization.nit || "—"}</td><td><span className="cell-stack"><span>{organization.sector || "Sin sector"}</span><small>{organization.city || "Sin ciudad"}</small></span></td><td className="numeric">{organization.campaign_count}</td><td>{formatDate(organization.created_at)}</td></tr>)}</tbody></table></div> : <EmptyState title="Sin organizaciones" text="Registra la primera empresa para habilitar sus campañas." />}
        </section>
